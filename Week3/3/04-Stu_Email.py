...
with open(csvpath,"w") as csvoutput:
    field_names = ["Last Name","First Name","SSN","Email"]
    writer = csv.DictWriter(csvoutput,fieldnames = field_names)
    writer.writeheader()
    for row3 in complete_list["First Name"]:
        writer.writerow(complete_list["First Name"][counter]+","+complete_list["Last Name"][counter]+","+str(complete_list["SSN"][counter])+","+str(complete_list["Email"][counter]))
